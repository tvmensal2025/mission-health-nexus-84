...
export const XiaomiScaleFlow: React.FC = () => {
  ...
  const connectToScale = async () => {
    try {
      ...
      // Configuração mais flexível para encontrar a balança
      ...
      if (device && device.gatt) {
        ...
        if (server) {
          ...
          // Aguardar dados reais da balança
          setCurrentStep('measuring');
          setIsWeighing(true);
          
          // Timeout de 10 segundos para receber dados
          setTimeout(() => {
            if (isWeighing) {
              console.log('Timeout de 10s - dados não recebidos, indo para pesagem manual');
              setCurrentStep('manual');
              setIsWeighing(false);
              toast({
                title: "Timeout - Indo para pesagem manual",
                description: 'Não foi possível receber dados da balança em 10s.',
                variant: "default"
              });
            }
